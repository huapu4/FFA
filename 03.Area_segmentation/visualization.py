...
parser.add_argument('--num_classes', help='The number of classes to be segmented.', default=3, type=int, required=True)
...
parser.add_argument('--color_encode', help='Whether to color encode the prediction.', default=True, required=True)
...
if args.csv_file is None:
    csv_file = os.path.join('./FFA_dataset', 'class_dict.csv')
else:
...
for i, name in enumerate(image_names):
    ...
    origin_image = cv2.resize(load_image(name),
                              dsize=(args.crop_width, args.crop_height))
    image = imagenet_utils.preprocess_input(origin_image.astype(np.float32), data_format='channels_last', mode='torch')
    ...
    if args.color_encode:
        prediction = np.uint8(color_encode(prediction, color_values))

    # add the prediction mask
    alpha, beta = 1, 0.5
    out = cv2.addWeighted(origin_image, alpha, prediction, beta, 0)
    ...
    out.save(os.path.join(paths['prediction_path'], file_name))
